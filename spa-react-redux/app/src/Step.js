...
import StepAPI from './apis/StepAPI';
...
export class StepForm extends Component {
  ...
  sendForm(event){
    ...
    this.props.save(this.state.name, this.state.description);
  }
      ...
      <div>
        ...
        <main role="main" className="container">
          ...
          <div className="row form-cadastro">
            <StepForm  save={this.props.save} store={this.context.store} />
          </div>
          <div className="row msg" role="alert">{this.props.msg}</div>
          <div className="row">
            <StepTable list={this.props.list} store={this.context.store}/>
          </div>
        </main>
      </div>
    );
...
StepBox.contextTypes = {
  store: PropTypes.object.isRequired
}

const mapStateToProps = state => {
  return {list : state.steps, msg: state.message}
};

const mapDispatchToProps = dispatch => {
  return {
    listSteps : () => {
      dispatch(StepAPI.getSteps());      
    },
    save: (name, description) => {
      dispatch(StepAPI.saveStep(name, description));      
    }
  }
}

const StepBoxContainer = connect(mapStateToProps,mapDispatchToProps)(StepBox);
